  ...
  return (
    <StyledStats>
        <div className="change-container">
          {
            Number(coin.delta_1h) < 0 ? (
              <h3 id="red"><span>1hr:</span> {Number(coin.delta_1h)}%</h3>
            ): (
              <h3><span>1hr:</span> {Number(coin.delta_1h)}%</h3>
            )
          }
          {
          ...
          {
            Number(coin.delta_7d) < 0 ? (
              <h3 id="red"><span>7d:</span> {Number(coin.delta_7d)}%</h3>
            ): (
              <h3><span>7d:</span> {Number(coin.delta_7d)}%</h3>
            )
          }
          {
            Number(coin.delta_30d) < 0 ? (
              <h3 id="red"><span>30d:</span> {Number(coin.delta_30d)}%</h3>
            ): (
              <h3><span>30d:</span> {Number(coin.delta_30d)}%</h3>
            )
          }
        </div>
